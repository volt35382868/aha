...
async def variable(event):  # sourcery no-metrics
    ...
    if cmd == "جلب":
        ...
        for i in configs:
            if variable in i:
                _, val = i.split("= ")
                return await cat.edit("𓆩 𝗦𝗼𝘂𝗿𝗰𝗲 𝗚𝗢𝗟𝗗 ⌁ - 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻" f"\n\n**⌔∮الفـار** `{variable} = {val}`")
        await cat.edit(
            "𓆩 𝗦𝗼𝘂𝗿𝗰𝗲 𝗚𝗢𝗟𝗗 ⌁ - 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻" f"\n\n**⌔∮الفـار :** -> {variable} **غيـر موجود**❌"
        )
    elif cmd == "ضع":
        ...
        for i in configs:
            if variable in i:
                ...
                string += f"{i}"
        if match:
            await cat.edit(f"**- تم تغيـر** `{variable}` **:**\n **- المتغيـر :** `{value}` \n**- يتم الان اعـادة تشغيـل بـوت جولد يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
        else:
            string += f"    {variable} = {value}\n"
            await cat.edit(
...
async def _(event):
    ...
    zed = await edit_or_reply(
        ...
        f"**⌔∮ يتـم الان اعـادة تشغيـل بـوت جولد فـي السيـرفـر قـد يستغـرق الامـر 2-3 دقيقـه ▬▭ ...**",
    )
    if cmd == "كلين":
        for file in exts:
