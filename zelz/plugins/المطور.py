...
ZelzalDV_cmd = (
    "[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗚𝗢𝗟𝗗 ⌁ - اوامــر المطـور المســاعد](t.me/oonvo) .\n\n"
...
async def add_sudo_user(event):
    ...
    output += "**⎉╎يتم الان اعـادة تشغيـل بـوت جــولد انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن 𝗚𝗢𝗟𝗗 ⌁ :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "آمن":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your oonvo__.\n"
                )
            else:
    ...
    output = (
        result + "**⎉╎يتم الان اعـادة تشغيـل بـوت جــولد انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
