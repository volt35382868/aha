...
video_opts = {
    ...
    "cookiefile": "/root/zelz/cookie/cookies.txt",  # إضافة ملف الكوكيز هنا
    "postprocessors": [
...
async def _(event): #Code by T.me/zzzzl1l
    ...
    'cookiefile': '/root/zelz/cookie/cookies.txt',  # تحديد ملف الكوكيز
    }
    try:
...
async def _(event): #Code by T.me/zzzzl1l
    ...
    "cookiefile": "/root/zelz/cookie/cookies.txt",  # إضافة ملف الكوكيز
    }
    try:
        results = YoutubeSearch(query, max_results=1).to_dict()
        link = f"https://youtube.com{results[0]['url_suffix']}"
...
ZelzalMeMe_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗚𝗢𝗟𝗗 ⌁ - اوامـر البصمـات 🎙](t.me/veevv2) 𓆪\n\n"
...
async def on_meme_list(event):
    ...
    for filt in filters:
        if OUT_STR == "**⪼ لا يوجـد لديك بصمـات محفوظـه ❌**\n\n**⪼ ارسـل (** `.بصمه` **) + اسم البصمـه**\n**⪼بالـرد ع بصمـه او مقطـع صـوتـي 🔊**\n**⪼ لاضافتهـا لـ قائمـة بصماتك 🧾**":
            OUT_STR = "𓆩 𝗚𝗢𝗟𝗗 ⌁ 𝗭𝗧𝗵𝗼𝗻 - قائمـة بصمـاتك المضـافـة 🔊𓆪\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        OUT_STR += "🎙 `{}`\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
...
async def on_meme_list(event):
    ...
    for filt in filters:
        if OUT_STR == "**⪼ لا يوجـد لديك ردود تلقائيـه لـ الخـاص ❌**\n\n**⪼ ارسـل (** `.اضف تلقائي` **) + كلمـة الـرد**\n**⪼بالـرد ع جملـة او ميديـا 🗣**\n**⪼ لاضافتهـا لـ قائمـة ردودك التلقائيـه 🧾**":
            OUT_STR = "𓆩 𝗦𝗼𝘂𝗿𝗰𝗲 𝗚𝗢𝗟𝗗 ⌁ - ردودك التلقـائيـه خـاص 🗣𓆪\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        OUT_STR += "🎙 `{}`\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
